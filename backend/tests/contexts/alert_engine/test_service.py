...
class AlertEngineService:
        ...
        for event in alert_events:
            ...
            for channel in channels:
                recipient = self._get_recipient_for_channel(
                    event.company_id, channel, preferences
                )

                # Skip if no real recipient found
                if not recipient:
                    logger.warning(
                        "no_recipient_for_channel",
                        trace_id=trace_id,
    ...
    def _get_recipient_for_channel(
        ...
        """Get real recipient for notification channel from preferences."""
        if not preferences:
            return None

        for pref in preferences:
            if channel == NotificationType.EMAIL and pref.email_enabled:
                # Use notification_email if set, otherwise fall back to user email
                if getattr(pref, 'notification_email', None):
                    return pref.notification_email
                if getattr(pref, 'email', None):
                    return pref.email
            elif channel == NotificationType.WHATSAPP and pref.whatsapp_enabled:
                if getattr(pref, 'whatsapp_number', None):
                    return pref.whatsapp_number
                if getattr(pref, 'phone_number', None):
                    return pref.phone_number
            elif channel == NotificationType.SMS and pref.sms_enabled:
                if getattr(pref, 'phone_number', None):
                    return pref.phone_number
