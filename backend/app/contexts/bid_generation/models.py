...
class BidGeneration(Base):
    ...
    competitiveness_score: Mapped[float] = mapped_column(Float, nullable=True)
    overall_quality_score: Mapped[float] = mapped_column(Float, nullable=True)
...
class BidGenerationAnalytics(Base):
    ...
    user_satisfaction_rate: Mapped[float] = mapped_column(Float, nullable=True)
    ...
    def __repr__(self) -> str:
        return f"BidGenerationAnalytics(id={self.id}, period={self.period_type})"
