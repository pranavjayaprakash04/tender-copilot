...
class VaultDocument(Base):
    ...
    # Relationships
    tender_mappings: Mapped[list[VaultDocumentMapping]] = relationship("VaultDocumentMapping", back_populates="document", cascade="all, delete-orphan")

    def __repr__(self) -> str:
        return f"VaultDocument(id={self.id}, company_id={self.company_id}, doc_type={self.doc_type}, filename={self.filename})"
...
class VaultDocumentMapping(Base):
    ...
    vault_doc_id: Mapped[UUID] = mapped_column(UUID, ForeignKey("vault_documents.id", ondelete="CASCADE"), primary_key=True)
    tender_id: Mapped[UUID] = mapped_column(UUID, ForeignKey("tenders.id", ondelete="CASCADE"), primary_key=True)
    ...
    # tender: Mapped["Tender"] = relationship("Tender", back_populates="document_mappings")
