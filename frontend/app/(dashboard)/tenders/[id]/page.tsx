    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Your Bid Amount (₹) <span className="text-red-500">*</span></label>
            <input type="number" value={form.bid_amount} onChange={e => setForm(f => ({ ...f, bid_amount: e.target.value }))}
              placeholder="Enter your bid amount"
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            {tender.estimated_value && <p className="text-xs text-gray-400 mt-1">Estimated value: {fmt(tender.estimated_value)}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">EMD Amount (₹)</label>
            <input type="number" value={form.emd_amount} onChange={e => setForm(f => ({ ...f, emd_amount: e.target.value }))}
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
              placeholder="Add any notes about this bid..." rows={3}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
          </div>
          {mutation.isError && <div className="bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg px-3 py-2">{(mutation.error as Error).message}</div>}
          {mutation.isSuccess && <div className="bg-green-50 border border-green-200 text-green-700 text-sm rounded-lg px-3 py-2">✓ Bid tracked! View it in your <a href="/bids" className="underline font-medium">Bid Pipeline</a>.</div>}
        </div>
        <div className="px-6 pb-5 flex gap-3">
          <button onClick={onClose} className="flex-1 border border-gray-200 text-gray-600 rounded-lg py-2.5 text-sm font-medium hover:bg-gray-50">Cancel</button>
          <button onClick={() => mutation.mutate()} disabled={!form.bid_amount || mutation.isPending || mutation.isSuccess}
            className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg py-2.5 text-sm font-medium hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed">
...
function WinRing({ probability }: { probability: number }) {
  const r = 44, circ = 2 * Math.PI * r, dash = circ * probability, color = winColor(probability);
  return (
    <svg width="110" height="110" viewBox="0 0 110 110">
      ...
      <text x="55" y="51" textAnchor="middle" fill={color} fontSize="18" fontWeight="700" fontFamily="sans-serif">{(probability * 100).toFixed(0)}%</text>
...
function WinProbabilityModal({ tender, companyId, onClose }: { tender: TenderDetail; companyId: string; onClose: () => void }) {
  ...
  const mutation = useMutation<WinProbabilityResponse, Error>({
    mutationFn: async () => {
      const res = await fetch("/api/win-probability", {
        ...
        body: JSON.stringify({
          tender_title: tender.title, tender_category: tender.category,
          estimated_value: tender.estimated_value, tender_location: tender.state,
          portal: tender.source, our_bid_amount: bidAmount ? parseFloat(bidAmount) : undefined,
  ...
  return (
    <Modal title="🎯 Win Probability" onClose={onClose}>
      ...
      {mutation.isError && <div style={{ background: "#EF444420", border: "1px solid #EF444440", borderRadius: 6, padding: "10px 14px", color: "#FCA5A5", fontSize: 12, marginTop: 12 }}>Analysis failed. Please try again.</div>}
      {!data && !mutation.isPending && <p style={{ color: "#475569", fontSize: 13, textAlign: "center", marginTop: 24 }}>Enter your bid amount and click Analyse</p>}
      {data && (
        <>
          <div style={{ display: "flex", gap: 20, alignItems: "center", marginTop: 16, flexWrap: "wrap" }}>
            <WinRing probability={data.win_probability} />
            <div style={{ flex: 1 }}>
              <span style={{ display: "inline-block", padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, background: winColor(data.win_probability) + "22", color: winColor(data.win_probability), marginBottom: 10 }}>{data.confidence?.toUpperCase()} CONFIDENCE</span>
              {data.market_avg && <p style={{ fontSize: 11, color: "#64748B", marginTop: 4 }}>Market average: {fmt(data.market_avg)}</p>}
            </div>
          </div>
          {data.recommended_range && (
            <div className="i-range">
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div><div style={{ fontSize: 16, fontWeight: 700, color: "#F1F5F9", fontFamily: "monospace" }}>{fmt(data.recommended_range.min)}</div><div style={{ fontSize: 10, color: "#64748B" }}>Min</div></div>
                <div style={{ color: "#1E2537" }}>→</div>
                <div><div style={{ fontSize: 16, fontWeight: 700, color: "#F1F5F9", fontFamily: "monospace" }}>{fmt(data.recommended_range.max)}</div><div style={{ fontSize: 10, color: "#64748B" }}>Max</div></div>
              </div>
            </div>
          )}
...
function CompetitorsModal({ tender, companyId, onClose }: { tender: TenderDetail; companyId: string; onClose: () => void }) {
    ...
    mutationFn: async () => {
      const res = await fetch("/api/competitors", {
        ...
        body: JSON.stringify({
          tender_id: tender.id,
          company_id: companyId,
          tender_title: tender.title,        // ← FIXED: was "title"
          tender_category: tender.category,  // ← FIXED: was "category"
  ...
  return (
    <Modal title="🏆 Competitor Analysis" onClose={onClose}>
      <style>{`.i-btn{padding:10px 20px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;border:none;background:#3B82F6;color:#fff}.i-btn:disabled{opacity:.4;cursor:not-allowed}.i-btn--outline{background:transparent;border:1px solid #1E2537;color:#94A3B8}.i-spinner{width:13px;height:13px;border:2px solid #1E2537;border-top-color:#3B82F6;border-radius:50%;animation:ispin .7s linear infinite;display:inline-block;vertical-align:middle;margin-right:6px}.comp-card{background:#1A1F2E;border:1px solid #1E2537;border-radius:8px;margin-bottom:8px;overflow:hidden}.comp-head{display:flex;align-items:center;gap:10px;padding:12px 14px;cursor:pointer}.comp-detail{display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:12px 14px;border-top:1px solid #1E2537}@keyframes ispin{to{transform:rotate(360deg)}}`}</style>
      {data && (
        <div style={{ background: "linear-gradient(135deg,#1E3A5F,#1A2540)", border: "1px solid #3B82F640", borderRadius: 8, padding: "14px 18px", marginBottom: 14, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontSize: 12, color: "#94A3B8" }}>Our Win Probability{data.recommended_price && <div style={{ fontSize: 11, color: "#64748B", marginTop: 2 }}>Recommended: {fmt(data.recommended_price)}</div>}</div>
          <div style={{ fontSize: 24, fontWeight: 700, fontFamily: "monospace", color: winColor(data.our_win_probability) }}>{pct(data.our_win_probability)}</div>
        </div>
      )}
      <button className={`i-btn${data ? " i-btn--outline" : ""}`} disabled={mutation.isPending} onClick={() => mutation.mutate()} style={{ marginBottom: 14 }}>
        {mutation.isPending ? <><span className="i-spinner" />Analysing…</> : data ? "Re-analyse" : "Analyse Competitors"}
      </button>
      {mutation.isError && <div style={{ background: "#EF444420", border: "1px solid #EF444440", borderRadius: 6, padding: "10px 14px", color: "#FCA5A5", fontSize: 12, marginBottom: 12 }}>Analysis failed. Please try again.</div>}
      {!data && !mutation.isPending && <p style={{ color: "#475569", fontSize: 13, textAlign: "center", marginTop: 16 }}>Click Analyse Competitors to see who you&apos;re up against</p>}
  ...
  return (
    <Modal title="📊 Market Price Intelligence" onClose={onClose}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div style={{ fontSize: 12, color: "#64748B" }}>Category: <strong style={{ color: "#E2E8F0" }}>{tender.category || "Not specified"}</strong></div>
        <button onClick={() => refetch()} disabled={!category || isLoading}
      ...
      {!data && !isLoading && <p style={{ color: "#475569", fontSize: 13, textAlign: "center", marginTop: 24 }}>Click Get Price Data to see market pricing</p>}
      {data && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
            {[{ label: "Average Price", value: fmt(data.avg_price), color: "#F1F5F9" }, { label: "Minimum (L1)", value: fmt(data.min_price), color: "#10B981" }, { label: "Maximum", value: fmt(data.max_price), color: "#EF4444" }, { label: "Tenders Sampled", value: data.sample_count.toString(), color: "#F59E0B" }].map(({ label, value, color }) => (
    ...
    mutationFn: async () => {
      const res = await fetch("/api/eligibility", {
        ...
        body: JSON.stringify({
          tender_title: tender.title, tender_category: tender.category,
          estimated_value: tender.estimated_value, tender_location: tender.state,
          portal: tender.source ?? "cppp", requirements: tender.description,
          company_name: profile?.name, company_industry: profile?.industry,
          company_location: profile?.location, gstin: profile?.gstin,
          udyam_number: profile?.udyam_number, turnover_range: profile?.turnover_range,
  ...
  return (
    <Modal title="✅ Eligibility Checker" onClose={onClose}>
      ...
      {!data && !mutation.isPending && (
        <>
          <p style={{ color: "#94A3B8", fontSize: 13, marginBottom: 16 }}>AI will check if <strong style={{ color: "#E2E8F0" }}>{profile?.name || "your company"}</strong> is eligible to bid on this tender.</p>
          <button onClick={() => mutation.mutate()} style={{ padding: "10px 20px", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer", border: "none", background: "#10B981", color: "#fff" }}>Check Eligibility</button>
        </>
      )}
      {mutation.isPending && <div style={{ textAlign: "center", padding: "32px 0", color: "#64748B", fontSize: 13 }}><div style={{ width: 32, height: 32, border: "3px solid #1E2537", borderTopColor: "#10B981", borderRadius: "50%", animation: "ispin .7s linear infinite", margin: "0 auto 12px" }} />Analysing eligibility…</div>}
...
function DocumentChecklistModal({ tender, onClose }: { tender: TenderDetail; onClose: () => void }) {
  ...
  const mutation = useMutation<DocumentChecklistResponse, Error>({
    mutationFn: async () => {
      const res = await fetch("/api/document-checklist", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ tender_id: tender.id, tender_title: tender.title, tender_category: tender.category, estimated_value: tender.estimated_value, tender_location: tender.state, description: tender.description }),
      });
    ...
    onSuccess: (data) => {
      ...
      data.checklist.forEach((item) => { if (item.status === "have" || item.in_vault) initial[item.id] = true; });
      setChecked(initial);
    },
  });
  const data = mutation.data;
  ...
  return (
    <Modal title="📋 Document Checklist" onClose={onClose}>
      <style>{`@keyframes ispin{to{transform:rotate(360deg)}}`}</style>
      {!data && !mutation.isPending && (<><p style={{ color: "#94A3B8", fontSize: 13, marginBottom: 16 }}>AI will generate a required document checklist for this tender.</p><button onClick={() => mutation.mutate()} style={{ padding: "10px 20px", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer", border: "none", background: "#6366F1", color: "#fff" }}>Generate Checklist</button></>)}
      {mutation.isPending && <div style={{ textAlign: "center", padding: "32px 0", color: "#64748B", fontSize: 13 }}><div style={{ width: 32, height: 32, border: "3px solid #1E2537", borderTopColor: "#6366F1", borderRadius: "50%", animation: "ispin .7s linear infinite", margin: "0 auto 12px" }} />Generating checklist…</div>}
      {mutation.isError && <div style={{ background: "#EF444420", border: "1px solid #EF444440", borderRadius: 6, padding: "10px 14px", color: "#FCA5A5", fontSize: 12, marginBottom: 12 }}>Failed to generate checklist.<button onClick={() => mutation.mutate()} style={{ marginLeft: 8, background: "none", border: "none", color: "#3B82F6", cursor: "pointer", fontSize: 12 }}>Retry</button></div>}
      {data && (
        <>
          <div style={{ background: readinessColor + "18", border: `1px solid ${readinessColor}40`, borderRadius: 10, padding: "14px 18px", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div><div style={{ fontSize: 13, fontWeight: 600, color: readinessColor }}>Readiness Score</div><div style={{ fontSize: 11, color: "#94A3B8", marginTop: 3 }}>{data.summary}</div></div>
            <div style={{ textAlign: "center" }}><div style={{ fontSize: 28, fontWeight: 800, color: readinessColor, fontFamily: "monospace" }}>{readiness}%</div><div style={{ fontSize: 10, color: "#64748B" }}>{checkedCount}/{total} docs</div></div>
          </div>
          <div style={{ height: 6, background: "#1E2537", borderRadius: 3, marginBottom: 16 }}><div style={{ height: 6, borderRadius: 3, background: readinessColor, width: `${readiness}%` }} /></div>
...
function PriceIntelligenceModal({ tender, companyId, onClose }: { tender: TenderDetail; companyId: string; onClose: () => void }) {
  const [bidAmount, setBidAmount] = useState(tender.estimated_value ? String(Math.round(tender.estimated_value * 0.92)) : "");
  const [hasAnalysed, setHasAnalysed] = useState(false);
  const mutation = useMutation<PriceIntelligenceResponse, Error>({
    mutationFn: async () => {
      const res = await fetch("/api/price-intelligence", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ tender_id: tender.id, tender_title: tender.title, tender_category: tender.category, estimated_value: tender.estimated_value, our_bid_amount: bidAmount ? parseFloat(bidAmount) : undefined }),
      });
  ...
  return (
    <Modal title="💰 Price Intelligence" onClose={onClose}>
      <style>{`.pi-input{width:100%;padding:9px 12px;background:#1A1F2E;border:1px solid #1E2537;border-radius:8px;color:#E2E8F0;font-size:13px;outline:none}.pi-input:focus{border-color:#F59E0B}.pi-btn{padding:10px 20px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;border:none;background:#F59E0B;color:#111;margin-top:10px}.pi-btn:disabled{opacity:.4;cursor:not-allowed}.pi-btn--sm{padding:7px 14px;font-size:12px;background:transparent;border:1px solid #1E2537;color:#94A3B8;margin-top:14px}.pi-spinner{width:13px;height:13px;border:2px solid #1E2537;border-top-color:#F59E0B;border-radius:50%;animation:pispin .7s linear infinite;display:inline-block;vertical-align:middle;margin-right:6px}.pi-card{background:#1A1F2E;border:1px solid #1E2537;border-radius:8px;padding:14px;margin-top:10px}.pi-section-title{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:#475569;margin-bottom:10px}@keyframes pispin{to{transform:rotate(360deg)}}`}</style>
      <div style={{ marginBottom: 4 }}>
        <label style={{ fontSize: 12, color: "#64748B", display: "block", marginBottom: 6 }}>Your intended bid amount <span style={{ color: "#F59E0B" }}>*</span></label>
        <input className="pi-input" type="number" placeholder="Enter your bid amount in ₹" value={bidAmount} onChange={e => setBidAmount(e.target.value)} />
        {tender.estimated_value ? <div style={{ fontSize: 11, color: "#475569", marginTop: 4 }}>Tender estimate: {fmt(tender.estimated_value)} · Pre-filled at 92%</div> : <div style={{ fontSize: 11, color: "#F59E0B", marginTop: 4 }}>⚠ No tender estimate — enter your bid amount</div>}
      </div>
      ...
      {mutation.isError && <div style={{ background: "#EF444420", border: "1px solid #EF444440", borderRadius: 6, padding: "10px 14px", color: "#FCA5A5", fontSize: 12, marginTop: 12 }}>Analysis failed. Please try again.</div>}
      {!hasAnalysed && !mutation.isPending && <p style={{ color: "#475569", fontSize: 13, textAlign: "center", marginTop: 28 }}>{bidAmount ? "Click Analyse Pricing to see market intelligence" : "Enter your bid amount and click Analyse Pricing"}</p>}
        ...
        <>
          {data.our_bid_amount && (
            <div style={{ background: scoreColor(data.price_to_win_score) + "14", border: `1px solid ${scoreColor(data.price_to_win_score)}35`, borderRadius: 10, padding: "16px 18px", marginTop: 16, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
              <div>
                ...
                {data.optimal_price && <div style={{ fontSize: 11, color: "#64748B", marginTop: 5 }}>Optimal target: <span style={{ color: "#F1F5F9", fontFamily: "monospace" }}>{fmt(data.optimal_price)}</span></div>}
              </div>
          ...
          <div className="pi-card">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
              {[{ label: "Market Min", value: fmtShort(data.market_min!), color: "#10B981" }, { label: "Market Avg", value: fmtShort(data.market_avg), color: "#F1F5F9" }, { label: "Market Max", value: fmtShort(data.market_max!), color: "#EF4444" }].map(({ label, value, color }) => (
                <div key={label} style={{ textAlign: "center" }}><div style={{ fontSize: 15, fontWeight: 700, color, fontFamily: "monospace" }}>{value}</div><div style={{ fontSize: 10, color: "#475569" }}>{label}</div></div>
              ))}
            </div>
          </div>
          {data.trend?.length > 0 && <div className="pi-card"><div className="pi-section-title">Price Trend — {data.category}</div><TrendChart trend={data.trend} /><div style={{ fontSize: 10, color: "#475569", marginTop: 4 }}>Blue line = market avg · Shaded area = min–max range</div></div>}
          {data.bands?.length > 0 && (
            <div className="pi-card">
              ...
              {data.bands.map((band: PriceBand, i: number) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderBottom: i < data.bands.length - 1 ? "1px solid #1E2537" : "none" }}>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}><div style={{ fontSize: 13, fontWeight: 700, color: bandColors[i], fontFamily: "monospace" }}>{(band.win_rate_estimate * 100).toFixed(0)}%</div><div style={{ fontSize: 9, color: "#475569" }}>est. win rate</div></div>
                </div>
              ))}
            </div>
          )}
...
export default function TenderDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: rawData, isLoading, error, refetch } = useQuery({
    ...
    enabled: !!tenderId,  // ← don't fetch if no id
    staleTime: 60_000,
  });

  const tender: TenderDetail | null = rawData ? ((rawData as any).data ?? rawData) as TenderDetail : null;
  ...
  if (!tenderId) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <p className="text-gray-600">Invalid tender ID</p>
    </div>
  );
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4">
            ...
            <div className="shrink-0 text-right">
              ...
              {tender.bid_submission_deadline && <p className={cn("text-sm font-medium mt-1", getDeadlineColor(tender.bid_submission_deadline))}>{getDaysLeft(tender.bid_submission_deadline)}</p>}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            {tender.source_url && (
              <a href={tender.source_url} target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center justify-center px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-medium hover:bg-indigo-700">
                View on Source Site ↗
              </a>
            )}
            <button onClick={() => setModal("eligibility")} className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-md text-sm font-medium hover:bg-emerald-700">✅ Check Eligibility</button>
            <button onClick={() => setModal("winprob")} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700">🎯 Win Probability</button>
            <button onClick={() => setModal("competitors")} className="inline-flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-md text-sm font-medium hover:bg-purple-700">🏆 Competitors</button>
            <button onClick={() => setModal("market")} className="inline-flex items-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-md text-sm font-medium hover:bg-orange-700">📊 Market Price</button>
