...
export default function TenderDetailPage({ params }: { params: { id: string } }) {
  ...
  const generateBidMutation = useMutation({
    mutationFn: async (tenderId: string) => {
      ...
      if (!response.ok) throw new Error("Failed to generate bid");
  ...
  const { data: bidStatus } = useQuery<BidStatusResponse>({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/bids/status/${taskId}`);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <MessageLoading />
          <p className="mt-4 text-gray-600">{t("tenders.loading")}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div className="flex flex-wrap gap-4 mb-4">
            ...
            <span className={cn(
              ...
              {t("tenders.deadline")}: {new Date(tender.deadline).toLocaleDateString(i18n.language === "ta" ? "ta-IN" : "en-IN")}
            </span>
          </div>
        </div>

        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">{t("tenders.description")}</h2>
          <p className="text-gray-700 leading-relaxed">{tender.description}</p>
        </div>

        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">{t("tenders.requirements")}</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            >
              {generateBidMutation.isPending || polling ? (
                <>
                  <MessageLoading />
                  <span className="ml-2">{t("tenders.generating_bid")}</span>
                </>
              ) : (
                t("tenders.generate_bid")
              )}
            </Button>
            ...
            <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-center">
                ...
                <span className="ml-2 text-blue-800">
                  {t("tenders.bid_progress")}: {bidStatus.progress}%
                </span>
              </div>
            </div>
          )}
